...
async def lightcurve_download(
    ...
    if format == "csv":
        buffer = io.StringIO()
        if band_name == "all":
            file_content = _transform_lc_to_csv(lightcurve=lightcurve_file, handle=buffer)
        else:
            file_content = _transform_band_lc_to_csv(lightcurve_band=lightcurve_file, handle=buffer)  
        buffer.close()
        media_type = "text/csv"
    elif format == "hdf5":
        buffer = io.BytesIO() 
        if band_name == "all":
            file_content = _transform_lc_to_hdf5(lightcurve=lightcurve_file, handle=buffer)
        else:
            file_content = _transform_band_lc_to_hdf5(lightcurve_band=lightcurve_file, handle=buffer)  
        buffer.close()
